-- Seed 50 farmers (we already have 5, so add 45 more)
...
SELECT 
  ...
  '+234' || (7000000000 + i * 1000000 + floor(random() * 999999))::text,
  ...
  'NIN-' || (10000000000 + i * 100000000)::text,
  NOW(),
  true,
  'pending',
  1
...
-- Seed farms (50 total, linked to farmers)
INSERT INTO farms (user_id, farmer_id, name, size, size_unit, location, latitude, longitude, soil_type, irrigation_type, created_at, updated_at, version)
SELECT 
  1,
  (i % 50) + 1,
...
-- Seed crops (50 total, linked to farms)
INSERT INTO crops (user_id, farm_id, name, variety, planting_date, expected_harvest_date, status, area, area_unit, created_at, updated_at, version)
SELECT 
  1,
  (i % 50) + 1,
...
-- Seed livestock (50 total, linked to farms)
INSERT INTO livestock (user_id, farm_id, type, breed, count, purpose, acquisition_date, health_status, created_at, updated_at, version)
SELECT 
  1,
  (i % 50) + 1,
...
-- Seed harvests (50 total, linked to crops)
INSERT INTO harvests (user_id, crop_id, quantity, unit, harvest_date, quality_grade, storage_location, notes, created_at, updated_at, version)
SELECT 
  1,
  (i % 50) + 1,
  (100 + i * 50)::numeric,
...
-- Seed expenses (50 total, linked to farms)
INSERT INTO expenses (user_id, farm_id, category, amount, description, expense_date, payment_method, receipt_url, created_at, updated_at, version)
SELECT 
  1,
  (i % 50) + 1,
...
-- Seed farm_inputs (50 total, linked to farms)
INSERT INTO farm_inputs (user_id, farm_id, type, name, quantity, unit, purchase_date, cost, supplier, created_at, updated_at, version)
SELECT 
  1,
  (i % 50) + 1,
