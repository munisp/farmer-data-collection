...
func initDB() {
	...
	if err != nil {
		log.Printf("[DB] Connection error for drone_service: %v", err)
		return
	}
	dbConn.SetMaxOpenConns(10)
	if err = dbConn.Ping(); err != nil {
		log.Printf("[DB] Ping failed for drone_service: %v (cache-only mode)", err)
		dbConn = nil
		return
	}
	log.Println("[DB] PostgreSQL connected for drone_service")
}
