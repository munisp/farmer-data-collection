import { pgTable, serial, integer, varchar, text, timestamp, boolean, index, unique, decimal } from "drizzle-orm/pg-core";
...
export const accountBalancesNew = pgTable("account_balances_new", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  accountCode: varchar("account_code", { length: 20 }).notNull(),
