...
export default function PaymentReconciliation() {
  const reconciliationQuery = trpc.notification.list.useQuery({ userId: 1, limit: 10 }, { retry: 1 });
  const reconciliationData = reconciliationQuery.data ?? null;
  ...
  const statusBadge = (status: string) => {
    switch (status) {
      case "reconciled": case "completed": return <Badge className="bg-green-100 dark:bg-green-900 text-green-800 dark:bg-green-900 dark:text-green-200"><CheckCircle className="h-3 w-3 mr-1" aria-hidden="true" />{status}</Badge>;
      case "pending": case "processing": return <Badge className="bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200"><Clock className="h-3 w-3 mr-1" aria-hidden="true" />{status}</Badge>;
      case "mismatched": return <Badge className="bg-red-100 dark:bg-red-900 text-red-800 dark:bg-red-900 dark:text-red-200"><AlertCircle className="h-3 w-3 mr-1" aria-hidden="true" />{status}</Badge>;
                ...
                <Table role="table" aria-label="Recent transactions" className="w-full">
                  ...
                  <TableBody role="rowgroup">
                    {recentTransactions.map((txn) => (
                      <TableRow key={txn.id} className="border-b dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700">
                        <TableCell className="p-3 font-mono text-sm dark:text-gray-300">{txn.id}</TableCell>
                        <TableCell className="p-3 dark:text-gray-300">{txn.order}</TableCell>
            ...
            <CardContent className="p-0">
              <div className="overflow-x-auto">
                <Table role="table" aria-label="Seller payouts" className="w-full">
                  ...
                  <TableBody role="rowgroup">
                    {payouts.map((p, i) => (
                      <TableRow key={i} className="border-b dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700">
                        <TableCell className="p-3 font-medium dark:text-white">{p.seller}</TableCell>
